...
bool Shunt::init(uint32_t shuntMicroOhm, uint16_t maximumAmps) {
    ...
    this->shuntMicroOhm = config.get<uint32_t>("shunt_micro_ohm", shuntMicroOhm);
    this->maximumAmps = config.get<uint16_t>("maximum_amps", maximumAmps);
...
float Shunt::getBusVoltage() {
    return ina.getBusMilliVolts(0) / 1000.0;
}
...
float Shunt::getPower() {
    return ina.getBusMicroWatts(0) / 1000000.0;
}

float Shunt::getStateOfCharge() {
    return calculateStateOfCharge();
}

void Shunt::setMaxCapacity(uint32_t ampHours) {
    maxCapacityMilliAmpMs = static_cast<int64_t>(ampHours) * 60LL * 60LL * 1000LL * 1000LL;

    ConfigManager& config = ConfigManager::getInstance();
    config.set("max_capacity", ampHours);
    config.saveConfig();
...
bool Shunt::loadConfig() {
    ...
    uint32_t maxCapacity = config.get<int>("max_capacity", 100); // Default 100Ah
    maxCapacityMilliAmpMs = static_cast<int64_t>(maxCapacity) * 60LL * 60LL * 1000LL * 1000LL;

    int socPercentage = config.get<int>("current_soc", config.get<int>("initial_soc", 80));
    currentCapacityMilliAmpMs = (maxCapacityMilliAmpMs / 100) * socPercentage;

    if (config.hasKey("current_capacity_mams")) {
        const char* capacityStr = config.get<const char*>("current_capacity_mams", "0");
        currentCapacityMilliAmpMs = atoll(capacityStr);
    }
...
bool Shunt::saveStateToConfig() {
    ...
    config.set("current_soc", socPercentage);
    ...
    config.set("current_capacity_mams", capacityStr);

    bool result = config.saveConfig();
...
void Shunt::update() {
    ...
    if (lastUpdateMillis > 0) {
        ...
        long elapsedMs = currentMillis - lastUpdateMillis;
        float currentAmps = getBusCurrent();
        int64_t capacityDeltaMilliAmpMs = static_cast<int64_t>(currentAmps * 1000 * elapsedMs);
    ...
    ConfigManager& config = ConfigManager::getInstance();
    unsigned long autoSaveInterval = config.get<int>("auto_save_interval", 30) * 1000;
...
float Shunt::calculateStateOfCharge() {
    ...
    return (static_cast<float>(currentCapacityMilliAmpMs) /
            static_cast<float>(maxCapacityMilliAmpMs)) * 100.0f;
}
